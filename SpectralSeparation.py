...
data = pd.read_csv('/Users/davidl./Biophotonics/Data/2M.csv')
df = data.drop(['Unnamed: 6'], axis=1)
#df1 = data1.drop(['Unnamed: 2'], axis=1)
wavelengths = df['empty'][1:][::-1].values.astype(int)
#nothing = df['Unnamed: 1'][1:][::-1].values.astype(float)
water1 = df['Unnamed: 3'][1:][::-1].values.astype(float)
glucose2M = df['Unnamed: 5'][1:][::-1].values.astype(float)
...
data3 = pd.read_csv('/Users/davidl./Biophotonics/Data/ham.csv')
hamWavelengths = data3['x'].values.astype(int)
hamAbsobance = data3['y'].values.astype(float)

data4 = pd.read_csv('/Users/davidl./Biophotonics/Data/opticaPaper.csv')
...
if __name__=="__main__":
    #print("Length of hamWavelengths:", len(hamWavelengths))
    #print("Length of ham:", len(ham))
    #two, ten = norm(glucose2M, glucoseSat)
    #print(transform(two), transform(ten))
    #separated_spectra_2M = isolate(WATER_RATIO_2, GLUCOSE_RATIO_2, glucose2M, water1, 0.02, 2)
    separated_spectra_10M = isolate(WATER_SAT, GLUCOSE_SAT, glucoseSat, water2, 0.02, 4.163)
    #print(separated_spectra_10M.shape)
    #divide = division(water, glucose)
    #plot(wavelengths, transform(separated_spectra_2M), transform(water1), transform(glucose2M))
    plot(wavelengths, transform(separated_spectra_10M), transform(water2), transform(glucoseSat), hamWavelengths, transform(hamAbsobance), opticaWavelengths, transform(opticaAbsorbance))
